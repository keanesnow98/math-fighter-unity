...
{
    /// <summary>
    /// Custom button for highlighting avaliable matches
    /// 
    /// NOTE: this particular button is used slightly differently from a normal menu type button.
    /// It should not have any north/south neighbours defined.  It will call back to the choosematch
    /// screen when up/down movement is recieved and the match list will determine the shift
    /// of focus from one button to the next.
    /// </summary>
    [TorqueXmlSchemaType]
    public class MFAvailableMatchHighlighter : GUIBasicButton, ITickObject
    {
        ...
        private GUIActionDelegate _action2; // the action that happens when the player presses downward on the gamepad
        private GUIActionDelegate _action3; // the action that happens when the player presses up on the gamepad
        ...
        public GUIActionDelegate Action3
        {
            get { return _action3; }
            set { _action3 = value; }
        }
        ...
        public override void  OnMoveUp()
        {
            ...
            if (Action3 != null) Action3(this, Params);
